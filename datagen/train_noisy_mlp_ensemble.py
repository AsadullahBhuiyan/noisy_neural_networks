...
# Use None for the full MNIST training set. Use an integer, e.g. 540, for a
...
def _load_clean_mnist_arrays(cfg: EnsembleTrainConfig, project_dir: Path):
...
def _folder_name(cfg: EnsembleTrainConfig, n_train: int, feature_dim: int) -> str:
    ...
    return (
        f"mlp_ensemble__{cfg.noise_type}__p={p_str}"
...
class DeterministicNoisyDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.noise_probability <= 0.0:
            noisy = x.copy()
        elif self.noise_type == "replacement":
            mask = rng.random(size=x.shape) < self.noise_probability
            ...
            epsilon = 1.0 - self.noise_probability
            gaussian = rng.normal(loc=0.0, scale=1.0, size=x.shape).astype(np.float32)
            noisy = epsilon * x + (1.0 - epsilon) * gaussian
        else:
            raise ValueError(
...
def train_ensemble(cfg: EnsembleTrainConfig, project_dir: Path) -> Path:
    ...
    x_train, y_train, x_test, y_test, train_indices = _load_clean_mnist_arrays(cfg, project_dir)
    ...
    metadata = {
        ...
        "test_is_clean": True,
        "normalization": "Each clean train/test example is normalized to unit RMS before noise is applied.",
    ...
    if cfg.save_test_outputs:
        ...
        metadata["test_outputs_rows"] = "One row per trained model and clean MNIST test example."
