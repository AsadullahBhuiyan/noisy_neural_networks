...
from src.nnet_models import build_model, compute_loss, evaluate, apply_corruption
...
def get_dataloaders_pristine(
    ...
    pristine_frac = max(0.0, min(1.0, pristine_frac))
    pristine_count = int(round(pristine_frac * len(train_base)))
    pristine_count = max(0, min(len(train_base), pristine_count))
    generator = torch.Generator().manual_seed(pristine_seed)
    perm = torch.randperm(len(train_base), generator=generator).tolist()
    pristine_indices = set(perm[:pristine_count])
...
def summarize_runs(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        key = (row["activation"], row["model_type"], float(row["pristine_frac"]), float(row["p"]))
        grouped.setdefault(key, []).append(row)

    summary_rows: list[dict] = []
    for (activation, model_type, pristine_frac, p_val), items in grouped.items():
        ...
        pristine_frac_actual = (pristine_total / train_set_size) if train_set_size else 0.0
        summary_rows.append(
            {
                ...
                "pristine_frac_actual": pristine_frac_actual,
                "repeats": len(items),
    ...
    summary_rows.sort(key=lambda r: (r["model_type"], r["activation"], r["p"], r["pristine_frac"]))
    return summary_rows
...
def train_and_evaluate_pristine(cfg: dict) -> dict:
    ...
    train_loader, train_eval_loader, test_loader, pristine_total = get_dataloaders_pristine(
        ...
        pristine_frac=cfg["pristine_frac"],
    ...
    for _ in tqdm(
        range(cfg["epochs"]),
        desc=f"epochs (w={cfg['mlp_width']}, pristine={cfg['pristine_frac']:.3f})",
        unit="epoch",
        leave=False,
...
def build_cache_key(
    ...
    f_min, f_max = min(pristine_fracs), max(pristine_fracs)
    p_min, p_max = min(p_values), max(p_values)
    return (
        f"{model_tag}_r{repeats}_e{epochs}_tf{test_fraction:.3f}_pristine{f_min:.3f}-{f_max:.3f}_"
        ...
        for model_type in model_types:
            ...
            for ax, act in zip(axes, activations):
                for frac in pristine_fracs:
                    d = sub[(sub["activation"] == act) & (sub["pristine_frac"] == frac)].sort_values("p")
                    if d.empty:
                        continue
                    frac_actual = d["pristine_frac_actual"].iloc[0]
                    ax.errorbar(
                        d["p"],
                        d["mean_test_accuracy"],
                        yerr=d["stderr_test_accuracy"],
                        marker="o",
                        label=f"pristine={frac:.3f} (actual={frac_actual:.3f})",
                    )
