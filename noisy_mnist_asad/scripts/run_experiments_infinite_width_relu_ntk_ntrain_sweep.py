...
def build_cache_key(
    ...
    p_min = min(p_values)
    p_max = max(p_values)
    n_min = min(n_train_values)
    n_max = max(n_train_values)
    return (
        f"ntk_relu_sweep_r{repeats}_ntr{n_min}-{n_max}_"
...
def main() -> None:
    ...
    p_values = np.linspace(0.75, 1.0, 51).tolist()
    n_train_values = [2048, 8192, 32768]
    ridge = 1e-6
    w_std = 1.0
    b_std = 1.0
    ...
    suffix = "infinite_width_relu_ntk_ntrain_sweep"
    seed = 1234
    ...
    pbar_n = tqdm(n_train_values, desc="n_train sweep", unit="n")
    for n_train in pbar_n:
        n_test = n_train
        print(
        ...
        pbar_p = tqdm(p_values, desc=f"p sweep @ n_train={n_train}", unit="p", leave=False)
        for p in pbar_p:
            accs: list[float] = []
            ...
            pbar_r = tqdm(
                ...
                desc=f"realizations @ n_train={n_train}, p={p:.3f}",
                unit="run",
                leave=False,
            )
    ...
    plot_path = os.path.join("results", "figures", f"accuracy_vs_p_{cache_key}{suffix_tag}.pdf")
    write_csv(per_run_path, rows_per_realization)
    write_csv(summary_path, rows_summary)

    summary_df = pd.DataFrame(rows_summary)
    plt.figure(figsize=(6, 4))
    for n_train in sorted(summary_df["n_train"].unique()):
        d = summary_df[summary_df["n_train"] == n_train].sort_values("p")
        plt.errorbar(
            d["p"],
            d["mean_test_accuracy"],
            yerr=d["stderr_test_accuracy"],
            marker="o",
            label=f"n_train=n_test={n_train}",
        )
    plt.xlabel("corruption probability p")
    plt.ylabel("mean test accuracy (clean test set)")
    plt.title(f"Infinite-width ReLU NTK n_train sweep (repeats={num_realizations})")
    ...
    append_runtime_log(
        runtime_log_path,
        {
            ...
            "total_runs": len(n_train_values) * len(p_values) * num_realizations,
            "n_train_values": n_train_values,
            "w_std": w_std,
            "b_std": b_std,
