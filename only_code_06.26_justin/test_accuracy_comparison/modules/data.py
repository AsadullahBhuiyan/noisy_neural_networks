...
class MNISTArrayDataset:
    ...
    def __getitem__(self, index: int) -> tuple[np.ndarray, int]:
        ...
        if image.shape != (height, width):
            ...
            pil_image = pil_image.resize((width, height), resample=resample_bilinear)
...
def normalize_dataset_name(name: str) -> str:
    ...
    if normalized not in _DATASET_DIR_NAMES:
        supported = ", ".join(sorted(_DATASET_DIR_NAMES))
        raise ValueError(f"Unsupported dataset {name!r}. Expected one of: {supported}.")
    if normalized in {"fashionmnist", "fmnist"}:
        return "fashion_mnist"
    return normalized


def dataset_dir_name(name: str) -> str:
    return _DATASET_DIR_NAMES[normalize_dataset_name(name)]
...
) -> tuple[MNISTArrayDataset, MNISTArrayDataset]:
    raw_dir = Path(root) / dataset_dir_name(dataset_name) / "raw"
...
def load_mnist(root: Path, img_hw: Tuple[int, int]) -> tuple[MNISTArrayDataset, MNISTArrayDataset]:
    return load_image_classification_dataset(root=root, img_hw=img_hw, dataset_name="mnist")
...
def dataset_to_numpy_matrix(dataset, num_classes: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    inputs = normalize_feature_matrix_zero_mean_unit_rms(inputs)
    return inputs, labels_onehot, labels


def normalize_feature_matrix_zero_mean_unit_rms(inputs: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    """
    ...
    Constant vectors are mapped to zero because both constraints cannot be
    satisfied when there is no variation across features.
    """
    inputs = np.asarray(inputs, dtype=np.float32)
    centered = inputs - np.mean(inputs, axis=1, keepdims=True)
    rms_sq = np.mean(centered * centered, axis=1, keepdims=True)
    scale = np.sqrt(np.maximum(rms_sq, np.float32(eps)))
    normalized = centered / scale
    zero_mask = rms_sq <= np.float32(eps)
    if np.any(zero_mask):
        normalized[zero_mask[:, 0]] = 0.0
    return normalized


def normalize_feature_matrix_unit_rms(inputs: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    return normalize_feature_matrix_zero_mean_unit_rms(inputs, eps=eps)


def corrupt_replacement_noise(inputs: np.ndarray, p: float, seed: int) -> np.ndarray:
    if p <= 0.0:
        return normalize_feature_matrix_zero_mean_unit_rms(inputs)
    ...
    return normalize_feature_matrix_zero_mean_unit_rms(noisy_inputs)
...
def corrupt_additive_gaussian_noise(inputs: np.ndarray, p: float, seed: int) -> np.ndarray:
    ...
    noisy_inputs = (epsilon * inputs) + ((1.0 - epsilon) * gaussian_noise)
    return normalize_feature_matrix_zero_mean_unit_rms(noisy_inputs)
