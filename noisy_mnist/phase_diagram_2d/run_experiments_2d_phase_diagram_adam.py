...
RUN_LOSS_TYPE = DEFAULT_LOSS_TYPE
RUN_TRAIN_LOG_INTERVAL = 100
RUN_TEST_LOG_INTERVAL = 500
RUN_PROGRESS_STEP_INTERVAL = 1000
RUN_MAX_WORKERS = 40
RUN_MAX_IN_FLIGHT = max(1, 9 * RUN_MAX_WORKERS // 10)
RUN_DATA_WORKERS = 0
RUN_CPU_THREADS_PER_WORKER = 1
RUN_CPU_CORE_START = 56
RUN_OUTPUT_ROOT = str(PROJECT_ROOT / "phase_diagram_2d_adam")
RUN_DATA_ROOT = "data"
RUN_SEED = 1234
...
def build_cache_key(
    ...
    return (
        f"phase2d_{OPTIMIZER_TAG}_{BALANCED_DATASET_TAG}_r{len(seeds)}_e{reference_epochs}_"
        f"p{p_min:.4f}-{p_max:.4f}_"
        f"b{b_min}-{b_max}_"
        f"tpc{tpc_min}-{tpc_max}_"
        f"w{width}_lr{learning_rate:.4g}_"
        f"wd{weight_decay:.4g}_loss-{loss_type}_"
...
@torch.no_grad()
def evaluate_clean(model: nn.Module, loader: DataLoader, device: torch.device, loss_type: str) -> tuple[float, float]:
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total_examples = 0
...
def run_single_spec(spec: RunSpec, cfg_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    total_steps = int(cfg.reference_epochs) * steps_per_epoch
    test_interval = max(1, min(cfg.test_log_interval, steps_per_epoch))
    progress_interval = max(1, cfg.progress_step_interval)

    train_dataset = CorruptedDataset(train_subset, p=spec.p, sigma=0.0, mode="replacement")
    ...
    model = build_model("mlp", "relu", mlp_hidden_sizes=[cfg.width]).to(device)
    ...
    for _epoch in range(cfg.reference_epochs):
        model.train()
        epoch_loss_sum = 0.0
        epoch_loss_weight = 0
        for x, y in train_loader:
            ...
            epoch_loss_sum += loss.item() * batch_size_now
            epoch_loss_weight += batch_size_now
            final_train_loss = float(epoch_loss_sum / max(1, epoch_loss_weight))
            ...
            if should_log(step, total_steps, test_interval):
                _, test_acc = evaluate_clean(model, test_loader, device, cfg.loss_type)
    ...
    final_test_loss, final_test_accuracy = evaluate_clean(model, test_loader, device, cfg.loss_type)
    if step_pbar is not None:
        step_pbar.close()
        ...
        rows.append(
            {
                ...
                "optimizer": str(row.get("optimizer", OPTIMIZER_TAG)),
                "learning_rate": float(row.get("learning_rate", DEFAULT_LEARNING_RATE)),
                "weight_decay": float(row.get("weight_decay", DEFAULT_WEIGHT_DECAY)),
                "loss_type": str(row.get("loss_type", DEFAULT_LOSS_TYPE)),
...
def build_config() -> ExperimentConfig:
    ...
    return ExperimentConfig(
        ...
        weight_decay=float(RUN_WEIGHT_DECAY),
        loss_type=str(RUN_LOSS_TYPE),
